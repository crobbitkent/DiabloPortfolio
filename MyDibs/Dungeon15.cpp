...
void Dungeon15::Init()
{
	mBGM = MyGameSound::SoundPlay(L"dlvlb.mp3");
}

void Dungeon15::MakeMap()
{
	Make3RoomsY();
...
void Dungeon15::SetMonster()
{
	SpawnMonster();
}

void Dungeon15::SpawnMonster()
{
	int size = 0;
	int ran = 0;
	int x = 0;
	...
	for (size_t i = 0; i < roomSize; ++i)
	{
		...
		for (int j = 0; j < size; ++j)
		{
			...
			case 2:
				sk = new Skeleton2(mMap1->ColMap(), mMap1, mPlayer, { x,y });
				break;
...
void Dungeon15::ResetPosFromTest()
{

	// Test에서의 플레이어 정보를 받아온다.
	Player* tempP = mTestScene->GetPlayer();
...
// 밑층에서 올라옴
void Dungeon15::ResetPosFrom18()
{

	// Dungeon15에서의 플레이어 정보를 받아온다.
