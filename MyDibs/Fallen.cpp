...
Fallen::Fallen(ColliderMap* colmap, RandomMap* randomMap, Player* player)
...
Fallen::Fallen(ColliderMap* colmap, RandomMap* randomMap, Player* player, TileIndex index)
	: Monster(colmap, randomMap)
	, mPlayer(player)
	, mIsPlayerAround(true)
	, mIsExpDone(false)
...
void Fallen::DebugRender()
{
	wchar_t Arr[256] = { 0, };

	swprintf_s(Arr, L"Fallen HP : %d / %d", mCurHP, mData.mMaxHP);
	TextOutW(MyTextureManager::BackDC(), 550, 0, Arr, lstrlen(Arr));

	swprintf_s(Arr, L"Fallen Index : X = %d , Y = %d", mTileIndex.X, mTileIndex.Y);
	TextOutW(MyTextureManager::BackDC(), 550, 15, Arr, lstrlen(Arr));

	swprintf_s(Arr, L"Fallen DibIndex : X = %d , Y = %d", mX, mY);
...
void Fallen::StateUpdate()
{
	...
	if (0 >= mCurHP && false == mIsDead)
	{
		...
		if (1 == ran)
		{
			MyGameSound::SoundPlay(L"falld1.wav");
		}
		else
		{
			MyGameSound::SoundPlay(L"falld2.wav");
		}
		...
		if (true == mGoingToAttack)
		{
			...
			if (nullptr != mDestDibTile->GetActor() || this != mDestDibTile->GetActor())
			{
				if (temp->GetTileIndex() == mDestDibTile->GetTileIndex())
				{
					...
					int ran = RandomNumber::GetRandomNumber(1, 4);
...
void Fallen::Attack()
{
	...
	else
	{
			...
			false == mAttacked)
		{
			...
			if (1 == rand)
			{
				MyGameSound::SoundPlay(L"falla1.wav");
			}
			else
			{
				MyGameSound::SoundPlay(L"falla2.wav");
			}
...
void Fallen::Special()
{
	...
	if (false == mIsAttacking)
	{
		...
		if (1 == rand)
		{
			MyGameSound::SoundPlay(L"falls1.wav");
		}
		else
		{
			MyGameSound::SoundPlay(L"falls2.wav");
		}
	...
	else
	{
		...
		if (0 != mAnimationRenderer->CurFrame() - mDir * 13 &&
			11 == (mAnimationRenderer->CurFrame() - mDir * 13) &&
			false == mAttacked)
		{
			mAttacked = true;

			mData.attackMin *= 1.3f;
			mData.toHit *= 1.3f;

			
		}
		else if (0 != mSubRenderer->CurFrame() - mDir * 13 &&
			11 == (mSubRenderer->CurFrame() - mDir * 13) &&
			false == mAttacked)
		{			
			mAttacked = true;
...
void Fallen::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void Fallen::GetDamage(int damage)
{
	int rand = RandomNumber::GetRandomNumber(1, 2);

	if (1 == rand)
	{
		MyGameSound::SoundPlay(L"fallh1.wav");
	}
	else
	{
		MyGameSound::SoundPlay(L"fallh2.wav");
...
void Fallen::SetAnimation()
{
	...
	mAllStateName[MS_IDLE] = L"F_IDLE";
	mAllStateName[MS_WALK] = L"F_WALK";
	mAllStateName[MS_ATTACK] = L"F_ATTACK";
	mAllStateName[MS_DIE] = L"F_DIE";
	mAllStateName[MS_GETHIT] = L"F_GETHIT";
	mAllStateName[MS_SPECIAL] = L"F_SPECIAL";

	AutoAnimation(L"IDLE", 10, 0.1f);
	AutoAnimation(L"WALK", 10);
	AutoAnimation(L"ATTACK", 12, mAttackSpeed);
	AutoAnimation(L"SPECIAL", 12);
	AutoAnimation(L"GETHIT", 10, 0.05f);
	AutoAnimation(L"DIE", 17, false, 0.05f);

	
}

// 가로 인덱스
void Fallen::AutoAnimation(const wchar_t * name, int index, float frameSpeed)
{
	// mAnimationRenderer->CreateAnimation(L"RB_F_IDLE", L"F&IDLE.bmp", 0, 12, true);
	// mAnimationRenderer->CreateAnimation(L"RB_F_GETHIT", L"F&GETHIT.bmp", 0, 7, true);

	int start = 0;
	int next = index;

	std::wstring fileName = L"F&";
	fileName.append(name);
	fileName.append(L".bmp");

	std::wstring blackName = L"F&";
...
void Fallen::AutoAnimation(const wchar_t * name, int index, bool loop, float frameSpeed)
{
...
void Fallen::GoingToMove(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void Fallen::GoingToAttack(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
