...
Butcher::Butcher(ColliderMap* colmap, RandomMap* randomMap, Player* player, TileIndex index)
...
void Butcher::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void Butcher::GetDamage(int damage)
{
	mCurHP -= damage;
...
// 가로 인덱스
void Butcher::AutoAnimation(const wchar_t * name, int index, float frameSpeed)
{
...
void Butcher::AutoAnimation(const wchar_t * name, int index, bool loop, float frameSpeed)
{
...
void Butcher::GoingToMove(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void Butcher::GoingToAttack(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
