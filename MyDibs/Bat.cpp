...
Bat::Bat(ColliderMap* colmap, RandomMap* randomMap, Player* player, TileIndex index)
...
void Bat::StateUpdate()
{
	...
	if (0 >= mCurHP && false == mIsDead)
	{
		...
		if (1 == ran)
		{
			MyGameSound::SoundPlay(L"batd1.wav");
		}
		else
		{
			MyGameSound::SoundPlay(L"batd2.wav");
		}
...
void Bat::Attack()
{
	...
	else
	{
			...
			false == mAttacked)
		{
			...
			if (1 == rand)
			{
				MyGameSound::SoundPlay(L"bata1.wav");
			}
			else
			{
				MyGameSound::SoundPlay(L"bata2.wav");
			}
...
void Bat::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void Bat::GetDamage(int damage)
{
	...
	if (1 == ran)
	{
		MyGameSound::SoundPlay(L"bath1.wav");
	}
	else
	{
		MyGameSound::SoundPlay(L"bath2.wav");
	}
...
// 가로 인덱스
void Bat::AutoAnimation(const wchar_t * name, int index, float frameSpeed)
{
...
void Bat::AutoAnimation(const wchar_t * name, int index, bool loop, float frameSpeed)
{
...
void Bat::GoingToMove(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void Bat::GoingToAttack(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
