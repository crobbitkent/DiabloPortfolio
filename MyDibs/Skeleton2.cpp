...
int Skeleton2::mCount = 0;

Skeleton2::Skeleton2(ColliderMap* colmap, RandomMap* randomMap, Player* player, TileIndex index)
...
void Skeleton2::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void Skeleton2::GetDamage(int damage)
{
	mCurHP -= damage;
...
// 가로 인덱스
void Skeleton2::AutoAnimation(const wchar_t * name, int index, float frameSpeed)
{
...
void Skeleton2::AutoAnimation(const wchar_t * name, int index, bool loop, float frameSpeed)
{
...
void Skeleton2::GoingToMove(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void Skeleton2::GoingToAttack(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
