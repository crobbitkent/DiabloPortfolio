...
void Dungeon18::SceneUpdate()
{
	...
	/*wchar_t Arr[256] = { 0, };
	swprintf_s(Arr, L"%f, %f", mouse.X, mouse.Y);
	TextOutW(MyTextureManager::BackDC(), 0, 80, Arr, lstrlen(Arr));*/


	//if (MyInput::Down(L"RMOUSEBTN"))
	//{
	//	mMap1->ColMap()->GetTile(mouse.X, mouse.Y);
	//}

	if (mPlayer->GetTileIndex().X == mMap1->GetDownPos().X && mPlayer->GetTileIndex().Y == mMap1->GetDownPos().Y)
	{
		mBGM->Stop();
		SetCurScene(L"Dungeon2");
		// 넘어갈때 2층 미리 설정
		mDungeon2->ResetPos();
	}
...
void Dungeon18::Init()
{
	mBGM = MyGameSound::SoundPlay(L"dlvlc.mp3");
}

void Dungeon18::MakeMap()
{
	Make3RoomsY();
...
void Dungeon18::ResetPosFrom15()
{

	// Dungeon18에서의 플레이어 정보를 받아온다.
...
void Dungeon18::ResetPosFrom2()
{

	// Dungeon18에서의 플레이어 정보를 받아온다.
	Player* tempP = mDungeon2->GetPlayer();
