...
int Skeleton::mCount = 0;

Skeleton::Skeleton(ColliderMap * colmap, RandomMap * randomMap, Player * player)
	...
	, mIsExpDone(false)
{
	...
	mAnimationRenderer->SubPos({ (float)0, (float)-32 +5 });
	mAnimationRenderer->SetCurrentAnimation(L"RB_SK_IDLE");

	mSubRenderer->SubSize({ 128, 128 });
	mSubRenderer->TransColor(255, 255, 255);
	mSubRenderer->SubPos({ (float)0, (float)-32+5 });
	mSubRenderer->SetCurrentAnimation(L"RB_SK_IDLE");

	mBlackRenderer->SubSize({ 128, 128 });
	mBlackRenderer->TransColor(255, 255, 255);
	mBlackRenderer->SubPos({ (float)0, (float)-32+5 });
	// mBlackRenderer->SetCurrentAnimation(L"RB_SK_IDLE");
...
Skeleton::Skeleton(ColliderMap* colmap, RandomMap* randomMap, Player* player, TileIndex index)
	: Monster(colmap, randomMap)
	, mPlayer(player)
	, mIsPlayerAround(true)
	, mIsExpDone(false)
...
void Skeleton::DebugRender()
{
	...
	swprintf_s(Arr, L"Skeleton HP : %d / %d", mCurHP, mData.mMaxHP);
	TextOutW(MyTextureManager::BackDC(), 550, 0, Arr, lstrlen(Arr));

	swprintf_s(Arr, L"Skeleton Index : X = %d , Y = %d", mTileIndex.X, mTileIndex.Y);
	TextOutW(MyTextureManager::BackDC(), 550, 15, Arr, lstrlen(Arr));

	swprintf_s(Arr, L"Skeleton DibIndex : X = %d , Y = %d", mX, mY);
...
void Skeleton::StateUpdate()
{
	...
	if (0 >= mCurHP && false == mIsDead)
	{
		...
		if (1 == ran)
		{
			MyGameSound::SoundPlay(L"sklsrd1.wav");
		}
		else
		{
			MyGameSound::SoundPlay(L"sklsrd2.wav");
		}
...
void Skeleton::Idle()
{
	...
	if (true == mAnimationRenderer->IsCurrentAnimationEnd())
	{
		...
		// mBlackRenderer->FinishAnimation();
	}
...
void Skeleton::Attack()
{
	...
	else
	{
			...
			false == mAttacked)
		{
			...
			if (1 == rand)
			{
				MyGameSound::SoundPlay(L"sklsra1.wav");
			}
			else
			{
				MyGameSound::SoundPlay(L"sklsra2.wav");
			}
...
void Skeleton::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void Skeleton::GetDamage(int damage)
{
	...
	if (1 == rand)
	{
		MyGameSound::SoundPlay(L"sklsrh1.wav");
	}
	else
	{
		MyGameSound::SoundPlay(L"sklsrh2.wav");
	}
...
void Skeleton::SetAnimation()
{
	...
	mAllStateName[MS_IDLE] = L"SK_IDLE";
	mAllStateName[MS_WALK] = L"SK_WALK";
	mAllStateName[MS_ATTACK] = L"SK_ATTACK";
	mAllStateName[MS_DIE] = L"SK_DIE";
	mAllStateName[MS_GETHIT] = L"SK_GETHIT";
	mAllStateName[MS_SPECIAL] = L"SK_SPECIAL";

	AutoAnimation(L"IDLE", 12, 0.05f);
	AutoAnimation(L"WALK", 7);
	AutoAnimation(L"ATTACK", 11, mAttackSpeed);
	AutoAnimation(L"SPECIAL", 15);
	AutoAnimation(L"GETHIT", 6);
	// AutoAnimation(L"DIE", 14);

	mAnimationRenderer->CreateAnimation(L"B_SK_DIE", L"SK&DIE.bmp", 0, 14, false, 0.05f);
	mAnimationRenderer->CreateAnimation(L"LB_SK_DIE", L"SK&DIE.bmp", 15, 29, false, 0.05f);
	mAnimationRenderer->CreateAnimation(L"L_SK_DIE", L"SK&DIE.bmp", 30, 44, false, 0.05f);
	mAnimationRenderer->CreateAnimation(L"LT_SK_DIE", L"SK&DIE.bmp", 45, 59, false, 0.05f);
	mAnimationRenderer->CreateAnimation(L"T_SK_DIE", L"SK&DIE.bmp", 60, 74, false, 0.05f);
	mAnimationRenderer->CreateAnimation(L"RT_SK_DIE", L"SK&DIE.bmp", 75, 89, false, 0.05f);
	mAnimationRenderer->CreateAnimation(L"R_SK_DIE", L"SK&DIE.bmp", 90, 104, false, 0.05f);
	mAnimationRenderer->CreateAnimation(L"RB_SK_DIE", L"SK&DIE.bmp", 105, 119, false, 0.05f);

	mSubRenderer->CreateAnimation(L"B_SK_DIE", L"SK&DIE.bmp", 0, 14, false, 0.05f);
	mSubRenderer->CreateAnimation(L"LB_SK_DIE", L"SK&DIE.bmp", 15, 29, false, 0.05f);
	mSubRenderer->CreateAnimation(L"L_SK_DIE", L"SK&DIE.bmp", 30, 44, false, 0.05f);
	mSubRenderer->CreateAnimation(L"LT_SK_DIE", L"SK&DIE.bmp", 45, 59, false, 0.05f);
	mSubRenderer->CreateAnimation(L"T_SK_DIE", L"SK&DIE.bmp", 60, 74, false, 0.05f);
	mSubRenderer->CreateAnimation(L"RT_SK_DIE", L"SK&DIE.bmp", 75, 89, false, 0.05f);
	mSubRenderer->CreateAnimation(L"R_SK_DIE", L"SK&DIE.bmp", 90, 104, false, 0.05f);
	mSubRenderer->CreateAnimation(L"RB_SK_DIE", L"SK&DIE.bmp", 105, 119, false, 0.05f);
	mBlackRenderer->Image(L"SK&DIE&BLACK.bmp", 0);
/*
	mBlackRenderer->CreateAnimation(L"B_SK_DIE", L"SK&DIE&BLACK.bmp", 0, 14, false, 0.05f);
	mBlackRenderer->CreateAnimation(L"LB_SK_DIE", L"SK&DIE&BLACK.bmp", 15, 29, false, 0.05f);
	mBlackRenderer->CreateAnimation(L"L_SK_DIE", L"SK&DIE&BLACK.bmp", 30, 44, false, 0.05f);
	mBlackRenderer->CreateAnimation(L"LT_SK_DIE", L"SK&DIE&BLACK.bmp", 45, 59, false, 0.05f);
	mBlackRenderer->CreateAnimation(L"T_SK_DIE", L"SK&DIE&BLACK.bmp", 60, 74, false, 0.05f);
	mBlackRenderer->CreateAnimation(L"RT_SK_DIE", L"SK&DIE&BLACK.bmp", 75, 89, false, 0.05f);
	mBlackRenderer->CreateAnimation(L"R_SK_DIE", L"SK&DIE&BLACK.bmp", 90, 104, false, 0.05f);
	mBlackRenderer->CreateAnimation(L"RB_SK_DIE", L"SK&DIE&BLACK.bmp", 105, 119, false, 0.05f);*/
}

// 가로 인덱스
void Skeleton::AutoAnimation(const wchar_t * name, int index, float frameSpeed)
{
	// mAnimationRenderer->CreateAnimation(L"RB_SK_IDLE", L"SK&IDLE.bmp", 0, 12, true);
	// mAnimationRenderer->CreateAnimation(L"RB_SK_GETHIT", L"SK&GETHIT.bmp", 0, 7, true);

	int start = 0;
	int next = index;

	std::wstring fileName = L"SK&";
	fileName.append(name);
	fileName.append(L".bmp");

	std::wstring blackName = L"SK&";
	...
	for (int i = 0; i < 8; ++i)
	{
		...
		// mBlackRenderer->CreateAnimation(aniName.c_str(), blackName.c_str(), start, next, true, frameSpeed);
...
void Skeleton::GoingToMove(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void Skeleton::GoingToAttack(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
