...
Zombie::Zombie(ColliderMap * colmap, RandomMap * randomMap, Player * player)
...
Zombie::Zombie(ColliderMap* colmap, RandomMap* randomMap, Player* player, TileIndex index)
	: Monster(colmap, randomMap)
	, mPlayer(player)
	, mIsPlayerAround(true)
	, mIsExpDone(false)
...
void Zombie::StateUpdate()
{
	...
	if (0 >= mCurHP && false == mIsDead)
	{
		...
		if (1 == rand)
		{
			MyGameSound::SoundPlay(L"zombied1.wav");
		}
		else
		{
			MyGameSound::SoundPlay(L"zombied2.wav");
		}
...
void Zombie::Attack()
{
	...
	else
	{
			...
			false == mAttacked)
		{
			...
			if (1 == rand)
			{
				MyGameSound::SoundPlay(L"zombiea1.wav");
			}
			else
			{
				MyGameSound::SoundPlay(L"zombiea2.wav");
			}
...
void Zombie::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void Zombie::GetDamage(int damage)
{
	...
	if (1 == rand)
	{
		MyGameSound::SoundPlay(L"zombieh1.wav");
	}
	else
	{
		MyGameSound::SoundPlay(L"zombieh2.wav");
...
// 가로 인덱스
void Zombie::AutoAnimation(const wchar_t * name, int index, float frameSpeed)
{
...
void Zombie::AutoAnimation(const wchar_t * name, int index, bool loop, float frameSpeed)
{
...
void Zombie::GoingToMove(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void Zombie::GoingToAttack(MyVector2 mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
